...
| **Minh Tiến** | **Bước 4: Modeling (Phần 1 - Tree-based Models)**<br>- Xây dựng Decision Tree (Baseline)<br>- Cấu hình & Tuning siêu tham số XGBoost Regressor bằng `RandomizedSearchCV` | - Soạn thảo Phần 3 (Mục XGBoost & Decision Tree).<br>- Lưu checkpoint `xgb_model.pkl` và xuất file nộp `sample_sub_xgb.csv`. |
